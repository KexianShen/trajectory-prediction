...
from .av2_extractor_multiagent import Av2ExtractorMultiAgent
...
class Av2Dataset(Dataset):
    def __init__(
        ...
        extractor: Av2ExtractorMultiAgent = None,
    ):
...
def collate_fn_cuda(batch):
    ...
    for key in [
        ...
        data[key] = pad_sequence([b[key] for b in batch], batch_first=True).to("cuda")
    ...
    if batch[0]["y"] is not None:
        data["y"] = pad_sequence([b["y"] for b in batch], batch_first=True).to("cuda")
    ...
    data["x_key_padding_mask"] = data["x_padding_mask"].all(-1).to("cuda")
    data["lane_key_padding_mask"] = data["lane_padding_mask"].all(-1).to("cuda")
    data["num_actors"] = (~data["x_key_padding_mask"]).sum(-1).to("cuda")
    data["num_lanes"] = (~data["lane_key_padding_mask"]).sum(-1).to("cuda")

    data["scenario_id"] = [b["scenario_id"] for b in batch]
    data["track_id"] = [b["track_id"] for b in batch]

    data["origin"] = torch.cat([b["origin"] for b in batch], dim=0).to("cuda")
    data["theta"] = torch.cat([b["theta"] for b in batch]).to("cuda")
