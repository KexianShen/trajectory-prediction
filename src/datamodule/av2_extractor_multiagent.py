...
class Av2ExtractorMultiAgent:
    ...
    def process(self, raw_path: str, agent_id: str = None):
        ...
        ego_df = df[df["track_id"] == agent_id].iloc
        origin = torch.tensor(
            [ego_df[49]["position_x"], ego_df[49]["position_y"]], dtype=torch.float
        )
        theta = torch.tensor([ego_df[49]["heading"]], dtype=torch.float)
